...
var logger = require('juttle/lib/logger').getLogger('elastic-backend');
    ...
    function query_fetcher() {
        return Promise.try(function() {
            if (should_execute_bridge_fetch) {
                ...
                logger.debug('executing query', url, JSON.stringify(body));
                return common.execute(url, body)
                    .then(function(result) {
                        var processed = process_query_result(result, body);
...
function fetcher_aggr_promise(es_url, filter, query_start, query_end,
                              aggregations, emit) {
    var url = common.build_logstash_url(es_url, query_start, query_end);
