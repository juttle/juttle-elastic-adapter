...
function make_reducer_agg(target_field, reducer, arg, logstash) {
    if (! _.has(REDUCERS_TO_ES_AGGRS, reducer)) {
        return null;
    }
    var aggr_name = REDUCERS_TO_ES_AGGRS[reducer];

    // 'count()' is the only zero-argument reducer we optimize, and
    // its argument defaults to 'time'
    if (arg === null) {
        arg = 'time';
    }
...
// do we have any "expensive" aggregations?  (currently defined
// as "do we have a cardinality/count_unique aggregation")
function _is_expensive(aggrs) {
    return _.some(aggrs, function(aggr) {
...
function make_bucket_agg(groupby, subagg, every, logstash) {
    var agg;
    ...
    var expensive = false;
    if (_.size(subagg) > 0) {
        agg.aggregations = subagg;
        expensive = _is_expensive(subagg);
    }

    agg.terms.size = expensive ? MAX_EXPENSIVE_AGGREGATION_BUCKETS :
        every ? MAX_GROUPED_TIME_BUCKETS :
...
function init(config) {
    ...
    MAX_GROUPED_TIME_BUCKETS = config.max_grouped_time_buckets || 500000;
}
